...
export const SmartChess = () => {
  ...
  const makeSmartAIMove = () => {
    ...
    // Profondeur 3 = IA assez forte
    const depth = 3;
    
    console.log('🤖 IA analyse', moves.length, 'coups possibles...');
    
    for (let move of moves) {
      chess.move(move);
      
      // L'IA joue les noirs, donc elle minimise (cherche le score le plus bas)
    ...
    console.log('🎯 Meilleur coup trouvé:', bestMove?.san, 'Score:', bestScore);
  ...
  useEffect(() => {
    if (chess.turn() === 'b' && !chess.isGameOver() && !isAIThinking) {
      ...
      // Délai plus long pour l'IA intelligente
      setTimeout(() => {
        makeSmartAIMove();
        setIsAIThinking(false);
      }, 1500); // 1.5 secondes pour la réflexion
    }
  ...
  const handleClick = (square) => {
    ...
    if (!selectedSquare) {
      ...
      try {
        ...
        if (move) {
          console.log('👤 Vous avez joué:', move.san);
          setSelectedSquare(null);
          setGameUpdate(prev => prev + 1);
        } else {
          setSelectedSquare(null);
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '30px' }}>
        ...
        <p style={{ fontSize: '1.2rem', margin: '0', color: '#d1d5db' }}>
          {chess.isGameOver() 
            ? (chess.isCheckmate() ? (chess.turn() === 'w' ? '🏆 AI Wins!' : '🏆 You Win!') : '🤝 Draw!')
            : isAIThinking 
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', flexWrap: 'wrap' }}>
        ...
        <div style={{
          ...
          <h3 style={{ margin: '0 0 20px 0' }}>🎮 Game Controls</h3>
          ...
          <div style={{
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#10b981' }}>🤖 AI Features</h4>
            <ul style={{ margin: '0', paddingLeft: '20px', fontSize: '0.9rem', lineHeight: '1.6' }}>
              <li>Minimax Algorithm (Depth 3)</li>
              <li>Alpha-Beta Pruning</li>
              <li>Position Evaluation</li>
              <li>Center Control Bonus</li>
              <li>Advanced Pawn Structure</li>
            </ul>
          </div>
